...
public final class MovieValidator {
    ...
    /**
     * @return null если валиден; иначе строка с описанием первой найденной ошибки
     */
    public static String validate(Movie m) {
        ...
        if (p.getHeight() == null || p.getHeight() <= 0) return "operator.height: должно быть > 0";
        if (p.getWeight() <= 0) return "operator.weight: должно быть > 0";

        Location loc = p.getLocation();
        if (loc != null) {
            if (loc.getX() == null) return "operator.location.x: не может быть null";
            if (isBlank(loc.getName())) return "operator.location.name: не может быть пустым";
        }
        return null;
    }
